...
Sample::Sample(int T)
{
	...
	T_ = T;
	internal_data_size_.resize((int)egmri::TOTAL_DATA_TYPES);
	internal_data_format_.resize((int)egmri::TOTAL_DATA_TYPES);
	meta_data_.resize((int)egmri::TOTAL_DATA_TYPES);
	// Fill in all possible sample types
	for(int i=0; i<egmri::TOTAL_DATA_TYPES; i++){
		internal_data_[(egmri::SampleType)i].resize(T);
...
void* Sample::get_data_pointer(int t, egmri::SampleType type)
{
    return NULL;
}

void Sample::set_data_vector(int t, egmri::SampleType type, double *data, int data_size, SampleDataFormat data_format)
{
    set_data_vector(t,type,data,data_size,1,data_format);
}

void Sample::set_data_vector(int t, egmri::SampleType type, double *data, int data_rows, int data_cols, SampleDataFormat data_format)
{
...
void Sample::set_data(int t, egmri::SampleType type, SampleVariant data, int data_size, SampleDataFormat data_format)
{
    if(t >= T_) ROS_ERROR("Out of bounds t: %d/%d", t, T_);
...
void Sample::get_data(int t, egmri::SampleType type, void *data, int data_size, SampleDataFormat data_format) const
{
    ROS_ERROR("Not supported!");
    return;
}

void Sample::set_meta_data(egmri::SampleType type, int data_size, SampleDataFormat data_format, OptionsMap meta_data)
{
    // A simplified version of set_meta_data for non-matrix types.
    set_meta_data(type, data_size, 1, data_format, meta_data);
}

void Sample::set_meta_data(egmri::SampleType type, int data_size_rows, int data_size_cols, SampleDataFormat data_format, OptionsMap meta_data)
{
...
void Sample::get_available_dtypes(std::vector<egmri::SampleType> &types){
    for(int i=0; i<egmri::TOTAL_DATA_TYPES; i++){
        if(internal_data_size_[i] != -1){
            types.push_back((egmri::SampleType)i);
        }
    }
}

void Sample::get_meta_data(egmri::SampleType type, int &data_size, SampleDataFormat &data_format, OptionsMap &meta_data_) const
{
...
void Sample::get_data_all_timesteps(Eigen::VectorXd &data, egmri::SampleType datatype){
	int size = internal_data_size_[(int)datatype];
	data.resize(size*T_);
	std::vector<egmri::SampleType> dtype_vector;
...
void Sample::get_data(int T, Eigen::VectorXd &data, egmri::SampleType datatype){
	int size = internal_data_size_[(int)datatype];
	data.resize(size*T);
	std::vector<egmri::SampleType> dtype_vector;
	dtype_vector.push_back(datatype);
...
void Sample::get_shape(egmri::SampleType sample_type, std::vector<int> &shape)
{
...
void Sample::get_data(int t, Eigen::VectorXd &data, std::vector<egmri::SampleType> datatypes)
{
    if(t >= T_) ROS_ERROR("Out of bounds t: %d/%d", t, T_);
    // Calculate size
    int total_size = 0;
