...
typedef std::map<egmri::SampleType, SampleList> SampleMap;
...
private:
    ...
    // State definition.
    std::vector<std::pair<egmri::SampleType,int> > state_definition_;
    // Observation definition.
    std::vector<std::pair<egmri::SampleType,int> > obs_definition_;
public:
    // Constructor.
    Sample(int T);
    ...
    virtual void get_meta_data(egmri::SampleType type, int &data_size, SampleDataFormat &data_format, OptionsMap &meta_data_) const;
    ...
    virtual void set_meta_data(egmri::SampleType type, int data_size, SampleDataFormat data_format, OptionsMap meta_data_);
    // Set sensor meta-data. Note that this resizes any fields that don't match the current format and deletes their data!
    virtual void set_meta_data(egmri::SampleType type, int data_size_rows, int data_size_cols, SampleDataFormat data_format, OptionsMap meta_data_);
    // Get datatypes which have metadata set
    virtual void get_available_dtypes(std::vector<egmri::SampleType> &types);

    // Get pointer to internal data for given time step.
    virtual void *get_data_pointer(int t, egmri::SampleType type);
    // Fill data arbitrary sensor information from a list of datatypes.
    virtual void get_data(int t, Eigen::VectorXd &data, std::vector<egmri::SampleType> datatypes);
    // Get sensor data for given timestep.
    virtual void get_data(int t, egmri::SampleType type, void *data, int data_size, SampleDataFormat data_format) const;
    // Get sensor data up to a given timestep, and for a particular datatype
    virtual void get_data(int T, Eigen::VectorXd &data, egmri::SampleType datatype);
    // Fill data with data for all timesteps from a single datatype
    virtual void get_data_all_timesteps(Eigen::VectorXd &data, egmri::SampleType datatype);

    // Add sensor data for given timestep.
    virtual void set_data(int t, egmri::SampleType type, SampleVariant data, int data_size, SampleDataFormat data_format);
    // Add sensor data for given timestep. Specialized version for Eigen matrix and vector data types.
    virtual void set_data_vector(int t, egmri::SampleType type, double *data, int data_size, SampleDataFormat data_format);
    // Add sensor data for given timestep. Specialized version for Eigen matrix and vector data types.
    virtual void set_data_vector(int t, egmri::SampleType type, double *data, int data_rows, int data_cols, SampleDataFormat data_format);


    // Fill shape with dimensions of data
    virtual void get_shape(egmri::SampleType sample_type, std::vector<int> &shape);
